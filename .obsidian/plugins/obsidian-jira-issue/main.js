        ...
        if (!value) {
          ...
          switch (key.trim().toLowerCase()) {
            ...
            case "limit":
              ...
              break;
            case "columns":
              this.columns = value.split(",").filter((column) => column.trim()).map((column) => {
                let columnExtra = "";
...
var JiraIssueSettingsTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Issue url to tags").setDesc(`Convert links to issues to tags. Example: ${this._data.host}/browse/AAA-123`).addToggle((toggle) => toggle.setValue(this._data.inlineIssueUrlToTag).onChange((value) => __async(this, null, function* () {
      this._data.inlineIssueUrlToTag = value;
      yield this.saveSettings();
    })));
    new import_obsidian.Setting(containerEl).setName("Inline issue prefix").setDesc(`Prefix to use when rendering inline issues. Keep this field empty to disable this feature. Example: ${this._data.inlineIssuePrefix}AAA-123`).addText((text) => text.setValue(this._data.inlineIssuePrefix).onChange((value) => __async(this, null, function* () {
    ...
    if (!query2.startsWith("$")) {
      for (const column of Object.values(ESearchColumnsTypes)) {
        if (suggestions.length >= this.limit)
          break;
        if (column.startsWith(query2) && column !== ESearchColumnsTypes.CUSTOM_FIELD) {
          suggestions.push({
            name: column,
            isCompact,
            isCustomField: false
...
var CountFenceRenderer = class {
  constructor(renderingCommon, client, cache) {
    ...
    return __async(this, null, function* () {
      ...
      if (cachedSearchResults) {
        if (cachedSearchResults.isError) {
          ...
          this.renderSearchCount(el, cachedSearchResults.data.total, source);
        }
      } else {
        ...
        this._client.getSearchResults(source, -1).then((newSearchResults) => {
          const searchResults = this._cache.add(source, newSearchResults).data;
  ...
  renderSearchCount(el, searchResultsCount, query2) {
    const tagsRow = createDiv("ji-tags has-addons");
    createSpan({ cls: `ji-tag is-link ${this._rc.getTheme()}`, text: `Count`, title: query2, parent: tagsRow });
    createSpan({ cls: `ji-tag ${this._rc.getTheme()}`, text: searchResultsCount.toString(), title: query2, parent: tagsRow });
    el.replaceChildren(this._rc.renderContainer([tagsRow]));
  }
};

// src/rendering/inlineIssueRenderer.ts
var InlineIssueRenderer = class {
  constructor(renderingCommon, settings, client, cache) {
    this._rc = renderingCommon;
    this._settings = settings;
    this._client = client;
    this._cache = cache;
      ...
      inlineIssueTags.forEach((value) => {
        ...
        if (cachedIssue) {
          ...
          this._client.getIssue(issueKey).then((newIssue) => {
            const issue = this._cache.add(issueKey, newIssue).data;
  ...
  convertInlineIssuesUrlToTags(el) {
    if (this._settings.inlineIssueUrlToTag) {
      const issueUrlElements = el.querySelectorAll(`a.external-link[href^="${this._settings.host}/browse/"]`);
      issueUrlElements.forEach((value) => {
...
var RenderingCommon = class {
  ...
  renderLoadingItem(item, itemUrl, inline = false) {
    let tagsRow;
    if (inline) {
...
var renderTableColumn = (columns, issue, row, renderingCommon) => {
...
function renderNoteFrontMatter(column, note, noteCell, renderingCommon) {
      ...
      try {
        ...
        if (cachedSearchResults) {
          ...
          this._client.getSearchResults(searchView.query, parseInt(searchView.limit) || this._settings.searchResultsLimit).then((newSearchResults) => {
            const searchResults = this._cache.add(searchView.query + searchView.limit, newSearchResults).data;
...
// src/rendering/searchWizardModal.ts
var import_obsidian4 = __toModule(require("obsidian"));
var SearchWizardModal = class extends import_obsidian4.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian4.Setting(contentEl).setName("Search rendering type").addDropdown((dropdown) => dropdown.addOptions(SEARCH_RESULTS_RENDERING_TYPE_DESCRIPTION).setValue(this._searchView.type).onChange((value) => __async(this, null, function* () {
      this._searchView.type = value;
    })));
    new import_obsidian4.Setting(contentEl).setName("Jira query").addTextArea((textArea) => textArea.setValue(this._searchView.query).onChange((value) => __async(this, null, function* () {
      this._searchView.query = value;
    })));
    new import_obsidian4.Setting(contentEl).setName("Search results limit").addText((text) => text.setPlaceholder(`Use default value: ${this._settings.searchResultsLimit}`).setValue(this._searchView.limit).onChange((value) => __async(this, null, function* () {
    ...
    new import_obsidian4.Setting(contentEl).setDesc(desc);
    this._searchView.columns.forEach((column, index) => {
      const setting = new import_obsidian4.Setting(contentEl).addDropdown((dropdown) => dropdown.addOptions(SEARCH_COLUMNS_DESCRIPTION).setValue(column.type).onChange((value) => __async(this, null, function* () {
    ...
    const searchColumnsButtons = new import_obsidian4.Setting(contentEl);
    searchColumnsButtons.addButton((button) => button.setButtonText("Add Column").setCta().onClick((value) => __async(this, null, function* () {
    ...
    new import_obsidian4.Setting(contentEl).addButton((btn) => btn.setButtonText("Insert").setCta().onClick(() => {
      this.close();
...
var InlineIssueWidget = class extends import_view.WidgetType {
  constructor(key, compact, renderingCommon, client, cache) {
    super();
    this._issueKey = key;
  ...
  buildTag() {
    ...
    if (cachedIssue) {
      ...
      this._client.getIssue(this._issueKey).then((newIssue) => {
        const issue = this._cache.add(this._issueKey, newIssue).data;
...
var matchDecorator;
function buildMatchDecorator(renderingCommon, settings, client, cache) {
  return new import_view.MatchDecorator({
    ...
    decoration: (match, view, pos) => {
      ...
      if (!view.state.field(import_obsidian5.editorLivePreviewField) || cursor > pos - 1 && cursor < pos + match[0].length + 1) {
...
var ViewPluginClass = class {
  constructor(view) {
    this.decorators = matchDecorator.createDeco(view);
  }
  update(update) {
    const editorModeChanged = update.startState.field(import_obsidian5.editorLivePreviewField) !== update.state.field(import_obsidian5.editorLivePreviewField);
...
var ViewPluginManager = class {
  constructor(renderingCommon, settings, client, cache) {
    ...
    this._cache = cache;
    this.update();
    this._viewPlugin = import_view.ViewPlugin.fromClass(ViewPluginClass, ViewPluginSpec);
  }
  update() {
